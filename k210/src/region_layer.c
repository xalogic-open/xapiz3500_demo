...
void region_layer_return_boxes(region_layer_t *rl, result_box_t *boxdata, callback_return_box callback) {
    uint32_t image_width= rl->image_width;
    uint32_t image_height= rl->image_height;
    float threshold= rl->threshold;
    box_t *boxes= (box_t *)rl->boxes;
    int j = 0;
    result_box_t *result = boxdata;
    ...
    for (int i= 0; i < rl->boxes_number; ++i) {
        ...
        if (prob > threshold) {
            box_t *b= boxes + i;
            result_box_t *rb = result + j;
            rb->x1= b->x * image_width - (b->w * image_width / 2);
            rb->y1= b->y * image_height - (b->h * image_height / 2);
            rb->x2= b->x * image_width + (b->w * image_width / 2);
            rb->y2= b->y * image_height + (b->h * image_height / 2);
            rb->boxclass = class;
            rb->prob = prob;
            j += 1;
             #if DEBUG_REGION_LAYER
                printf ("Boxes valid %d\n",j);
                printf ("X1 : %d\n",rb->x1);
                printf ("Y1 : %d\n",rb->y1);
                printf ("X2 : %d\n",rb->x2);
                printf ("Y2 : %d\n",rb->y2);
